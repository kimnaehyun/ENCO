...
type SettleMemberSelectNavigationProp = NativeStackNavigationProp<
  GroupStackParamList,
  'SettleMemberSelect'
>;
type SettleMemberSelectRouteProp = RouteProp<
  GroupStackParamList,
  'SettleMemberSelect'
>;
...
export default function SettleMemberSelectScreen() {
  const navigation = useNavigation<SettleMemberSelectNavigationProp>();
  const route = useRoute<SettleMemberSelectRouteProp>();
  const params =
    route.params ??
    ({} as NonNullable<GroupStackParamList['SettleMemberSelect']>);
  ...
  useEffect(() => {
    ...
    if (!Number.isFinite(numericGroupId)) {
      setMemberLoadError(
        '유효한 모임 ID가 없어 정산 대상자를 불러올 수 없습니다.',
      );
      return;
    }
  ...
  const navigateToCreatedSettlementDetail = (
    expenseId: number,
    ...
    const preservedRoutes = existingRoutes.filter(
      (route: { name?: string }) =>
        route.name !== 'SettlementReceiptOcr' &&
        route.name !== 'SettleMemberSelect',
    );
  ...
  const handleRegister = () => {
    ...
    if (!params.receiptDraft) {
      Alert.alert(
        '안내',
        '검수된 영수증 데이터가 없습니다. OCR 검수부터 진행해 주세요.',
      );
      return;
    }
    ...
    if (!params.receiptUri) {
      Alert.alert(
        '안내',
        '영수증 스캔본이 없습니다. 영수증 이미지를 다시 선택해 주세요.',
      );
      return;
    }
  ...
  const [members] = useState<SettleMember[]>(
    params.settleMembers ?? FALLBACK_SETTLE_MEMBERS,
  );
  ...
  const existingPerPerson =
    members.length > 0 ? Math.ceil(amount / members.length) : 0;
        ...
        { text: '취소', style: 'cancel' },
        {
          ...
          onPress: async () => {
            const numericExpenseId =
              typeof params.expenseId === 'number' ? params.expenseId : NaN;
        ...
        >
          ...
          <View style={styles.guideCard}>
            ...
            <Text style={styles.guideText}>
              영수증 검수값을 바탕으로 표시명만 기본 입력했습니다. 메모와 입금
              계좌를 확인한 뒤, 정산을 요청할 인원을 선택하세요.
            </Text>
          </View>
            ...
            <View style={styles.memberSectionHeader}>
              <View>
                ...
                <Text style={styles.memberSectionHelper}>
                  선택한 인원에게 1인당 {perPerson.toLocaleString()}원이
                  청구됩니다.
                </Text>
              </View>
                ...
                selectableMembers.map(member => {
                  ...
                  return (
                    <Pressable
                      ...
                      style={[
                        styles.newMemberRow,
                        isSelected && styles.newMemberRowSelected,
                      ]}
                    >
                      <View
                        style={[
      ...
      >
        ...
        >
          ...
          <View className="flex-row items-center justify-between">
            ...
            <Text style={styles.summaryPerPersonBlue}>
